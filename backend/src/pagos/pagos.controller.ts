...
import { ErrorNegocio } from "../aprobaciones/aprobaciones.service";
...
/** POST /facturas/:facturaId/pagos — body: { monto, fecha_pago?, metodo_pago?, observaciones?, codigo_otp } */
export async function registrar(req: Request, res: Response): Promise<void> {
  try {
    const facturaId = obtenerFacturaId(req);
    const usuario = req.usuario!;
    const resultado = await service.registrarPago(facturaId, usuario.id, req.body);
